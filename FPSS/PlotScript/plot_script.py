...
if plot_heating_curve:
    ...
    plt.scatter(df["DataInput.T_amb_mod"]-273.15, df["tGA_one_zone_simple.SecondaryCircuit.ctr_HP.heatingCurve_2s.TSupSet"]-273.15,  label='Heating Curve')
    plt.xlabel("$T_{amb}$ [°C]")
    plt.ylabel("$T_{sup}$ [°C]")
...
if plot_controller:
    ...
    axs[0,1].plot(time, df['DataInput.T_amb_mod']-273.15, label="$T_{amb}$ [°C]")
    axs[0,1].legend()
...
if plot_miscellaneous:
    ...
    axs[0,0].plot(time, df['DataInput.T_amb_mod']-273.15, label="$T_{amb}$ [°C]")
    axs[0,0].legend()
