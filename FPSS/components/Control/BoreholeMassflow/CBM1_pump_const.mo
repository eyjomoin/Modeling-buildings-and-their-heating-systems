...
equation
  ...
  connect(add.y, PID.u_m) annotation(
    Line(points = {{-39, -30}, {0, -30}, {0, -12}}, color = {0, 0, 127}));
  connect(PID.y, signal_inverter.controler_signal) annotation(
    Line(points = {{11, 0}, {42, 0}}, color = {0, 0, 127}));
  connect(Control_m_flow, signal_inverter.inverted_signal) annotation(
    Line(points = {{120, 0}, {66, 0}}, color = {0, 0, 127}));
  annotation (
    Documentation(info = "<html><head></head><body>Delta T between T_Bor_in and T_Bor_out is controlled to a value set in the parameter file. (3 is a common value)<div><br></div></body></html>"));
end CBM1_pump_const;
