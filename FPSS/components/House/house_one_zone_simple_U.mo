...
model house_one_zone_simple_U
  package MediumAir = AixLib.Media.Air "Medium model for air";
  replaceable parameter Parameter.Building.Building_base Parameter_building          constrainedby
    Parameter.Building.Building_base                                                                                                annotation (
  ...
  Building_component.house_component_U wall(
    CA=Parameter_building.A_wall,
    Cd=Parameter_building.dWall,
    Ccp=Parameter_building.cpWall,
    Crho=Parameter_building.rhoWall,
    CU=Parameter_building.UWall,
    Ch=Parameter_building.hWall,
    T_start=Parameter_building.T_zone_start) annotation (Placement(
        transformation(origin={-44,90}, extent={{-13,-12},{13,12}})));
  Building_component.house_component_U roof(
    CA=Parameter_building.A_roof,
    Cd=Parameter_building.droof,
    Ccp=Parameter_building.cproof,
    Crho=Parameter_building.rhoroof,
    CU=Parameter_building.Uroof,
    Ch=Parameter_building.hroof,
    T_start=Parameter_building.T_zone_start) annotation (Placement(
        transformation(origin={-42,60}, extent={{-13,-12},{13,12}})));
  Building_component.house_component_U ground(
    CA=Parameter_building.A_ground,
    Cd=Parameter_building.dground,
    Ccp=Parameter_building.cpground,
    Crho=Parameter_building.rhoground,
    CU=Parameter_building.Uground,
    Ch=Parameter_building.hground,
...
equation
  ...
  connect(rad_R1, roof.rad) annotation (
    Line(points={{-120.5,75.5},{-80,75.5},{-80,59.95},{-55.05,59.95}}));
  connect(rad_R1, ground.rad) annotation (
    Line(points={{-120.5,75.5},{-80,75.5},{-80,25.95},{-55.05,25.95}}));
  connect(rad_R1, wall.rad) annotation (
    Line(points={{-120.5,75.5},{-80,75.5},{-80,89.95},{-57.05,89.95}}));
  connect(wall.conv, zon.heatPort) annotation (
    Line(points={{-31.05,83.95},{0,83.95},{0,-59},{112,-59}}, color = {191, 0, 0}));
  connect(roof.conv, zon.heatPort) annotation (
    Line(points={{-29.05,53.95},{0,53.95},{0,-59},{112,-59}}, color = {191, 0, 0}));
  connect(ground.conv, zon.heatPort) annotation (
    Line(points={{-29.05,19.95},{0,19.95},{0,-59},{112,-59}}, color = {191, 0, 0}));
