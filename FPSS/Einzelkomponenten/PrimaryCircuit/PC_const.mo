within FPSS.Einzelkomponenten.PrimaryCircuit;
model PC_const
  ...
  Control.BoreholeMassflow.CBM1_pump_const cbm1(mflow_min=Parameter_TGA.mflow_prim_min,
      mflow_max=Parameter_TGA.mflow_prim_max) annotation (Placement(
...
</body></html>", info = "<html><head></head><body><p>Model of the primary circuit of the water to water heat pump. In the experiments the HeatPump is supplied by 3 U-borehole-probes and one spiral probe.</p><p>In this model the Heat Pump is supplied by an infinitive source of brine with the temperature of the experiments. The pump delivers exactly the flow rate measured in the experiments.</p><ul>
</ul>
</body></html>"),
