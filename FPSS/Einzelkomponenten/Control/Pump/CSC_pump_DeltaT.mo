...
model CSC_pump_DeltaT
  parameter Modelica.Units.SI.MassFlowRate mflow_min = 0 "minimum massflow rate. Possibly necessary for controlling purposes.";
  parameter Modelica.Units.SI.MassFlowRate mflow_max = 0.3 "Nominal mass flow rate for water loop. Maximum Value found in data was 0.982 m3/h = 0.278233 kg/s";
  ...
  AixLib.Controls.Continuous.LimPID PID_pump_sec(
    ...
    Ti(displayUnit="s") = 200,
    Td=0.01,                                                                                                                                                                                           Ni = 0.1,
    k=0.01,                                                                                                                                                                                                       Nd = 50, wp = 0, wd = 0) annotation (
  ...
  Modelica.Blocks.Interfaces.RealInput Temperature_supply(unit="K") annotation (Placement(
        ...
        extent={{20,-20},{-20,20}},
        rotation=180)));
  Modelica.Blocks.Interfaces.RealOutput control_signal_pump(unit="kg/s") annotation (
  ...
  signal_inverter_min_max signal_inverter_min_max1(ctr_max=mflow_max, ctr_min=
        mflow_min)
...
equation
  ...
  connect(PID_pump_sec.y, signal_inverter_min_max1.controler_signal)
    annotation (Line(points={{11,72},{34,72}}, color={0,0,127}));
  connect(signal_inverter_min_max1.inverted_signal, control_signal_pump)
    annotation (Line(points={{58,72},{80,72},{80,0},{110,0}}, color={0,0,127}));
annotation (
