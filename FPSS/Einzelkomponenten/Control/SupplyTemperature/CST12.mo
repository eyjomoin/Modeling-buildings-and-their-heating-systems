...
model CST12
  ...
  AixLib.Controls.Continuous.LimPID PID_HP( controllerType = Modelica.Blocks.Types.SimpleController.PID, yMax = 1, yMin = 0, wp = 1, k = 0.2, Ti = 300, Td = 22, Ni = 60, Nd = 0.1) annotation (
  ...
  Modelica.Blocks.Logical.OnOffController onOffController(bandwidth = 4)  annotation (
  ...
  Modelica.Blocks.Math.BooleanToReal booleanToReal(realTrue = 0.2, realFalse = 0)  annotation (
    Placement(transformation(origin = {10, -18}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  ...
  Switch.delay delay(perio = 840)  annotation (
    Placement(transformation(origin = {-10, -28}, extent = {{-6, -6}, {6, 6}}, rotation = -90)));
  Modelica.Blocks.Math.Add add annotation (
    Placement(transformation(origin = {16, 59}, extent = {{-5, -5}, {5, 5}}, rotation = -90)));
  Modelica.Blocks.Sources.Constant const1(k = 0)  annotation (
    Placement(transformation(origin = {-6, 68}, extent = {{-4, -4}, {4, 4}})));
equation
  ...
  connect(booleanToReal.y, switch2.u1) annotation (
    Line(points = {{10, -29}, {10, -51.5}, {-2, -51.5}, {-2, -58}}, color = {0, 0, 127}));
  connect(onOffController.y, booleanToReal.u) annotation (
    Line(points = {{10, 0}, {10, -6}}, color = {255, 0, 255}));
  connect(switch2.y, Control_Signal_Compressor) annotation (
    Line(points = {{-10, -80}, {-10, -88}, {0, -88}, {0, -110}}, color = {0, 0, 127}));
  connect(lessThreshold02.y, switch1.u2) annotation (
    Line(points = {{-30, 0}, {-30, -6}, {-54, -6}, {-54, -16}}, color = {255, 0, 255}));
  connect(const.y, switch1.u1) annotation (
    Line(points = {{-76, -12}, {-46, -12}, {-46, -16}}, color = {0, 0, 127}));
  connect(PID_HP.y, switch1.u3) annotation (
    Line(points = {{64, 80}, {80, 80}, {80, 48}, {-62, 48}, {-62, -16}}, color = {0, 0, 127}));
  connect(switch1.y, switch2.u3) annotation (
    Line(points = {{-54, -38}, {-54, -46}, {-18, -46}, {-18, -58}}, color = {0, 0, 127}));
  connect(delay.y, switch2.u2) annotation (
    Line(points = {{-10, -34}, {-10, -58}}, color = {255, 0, 255}));
  connect(lessThreshold02.y, delay.u) annotation (
    Line(points = {{-30, 0}, {-30, -6}, {-10, -6}, {-10, -22}}, color = {255, 0, 255}));
  connect(const1.y, add.u2) annotation (
    Line(points = {{-2, 68}, {14, 68}, {14, 66}}, color = {0, 0, 127}));
  connect(Heating_Curve.TSup, add.u1) annotation (
    Line(points = {{-18, 80}, {20, 80}, {20, 66}}, color = {0, 0, 127}));
  connect(add.y, onOffController.reference) annotation (
    Line(points = {{16, 54}, {16, 22}}, color = {0, 0, 127}));
  annotation (
